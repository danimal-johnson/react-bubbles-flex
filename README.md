...
* A token is used to authenticate a user when interacting with an API. It is issued by the server after a successful login and sent with each subsequent request.

- [x] What steps can you take in your web apps to keep your data secure?

* This is a huge topic. The method this sprint focused on was providing private or protected routes that aren't accessible when a web token isn't present.

- [x] Describe how web servers work.

* A web server (as software) runs on a machine and listens to a port (80 or 443 by default for the web) and responds to requests. An API can serve any information and be run on any port, however. The hardware can be anything, from an IOT device to a data center.
